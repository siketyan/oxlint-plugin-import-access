...
function isPackageDirectory(
  ...
  for (const pattern of patterns) {
    if (pattern.startsWith("!")) {
      ...
      if (minimatch(dirName, positivePattern) || minimatch(relativePath, positivePattern)) {
        return false;
      }
    } else {
 ...
 */
function getPackageDirectory(filePath: string, packageOptions: PackageOptions): string {
