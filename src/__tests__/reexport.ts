...
describe("reexports", () => {
  ...
  it("Cannot re-export a package-private variable", async () => {
    const result = await tester.lintFile("src/reexport4/indexLoophole/reexportFromSubFoo.ts");
    expect(result).toMatchInlineSnapshot(`
      Array [
        Object {
          "column": 10,
  ...
  it("Can re-export a variable exported from index.ts", async () => {
    const result = await tester.lintFile("src/reexport4/indexLoophole/reexportFromSubIndex.ts");
  ...
  describe("filenameLoophole = true", () => {
    ...
    it("Can re-export from sub directory of same name", async () => {
      const result = await tester.lintFile("src/reexport4/filenameLoophole/sub.ts", {
        jsdoc: {
          indexLoophole: false,
          filenameLoophole: true,
        },
    ...
    it("Cannot re-export from sub directory of different name", async () => {
      const result = await tester.lintFile("src/reexport4/filenameLoophole/sub2.ts", {
        jsdoc: {
          indexLoophole: false,
          filenameLoophole: true,
        },
